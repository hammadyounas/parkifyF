...
//       await axios.delete(
//         'https://parkify-backend.vercel.app/user/cancelParking',
//         {
//           data: { parkingId },
//           headers: {
...
//         await axios.get(
//           `https://parkify-backend.vercel.app/user/bookedparking?email=${Cookies.get('email')}`,
//           {
//             headers:{
//               Authorization: `bearer ${Cookies.get('token')}`,
...
const UserBookings = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`https://parkify-backend.vercel.app/user/bookedparking?email=${Cookies.get('email')}`, {
          headers: {
            Authorization: `bearer ${Cookies.get('token')}`,
          },
        });
        const newData = response.data.data.map((item) => ({
          ...
          endDate: item.endDate,
          cancelbooking: (
            <Button type="primary" danger onClick={() => handleCancelButton(item.key)}>
              Cancel
            </Button>
          ),
        }));
